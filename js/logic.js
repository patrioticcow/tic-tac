...
function createHtml(g, grid, h, w) {
    var s    = (g - 4);
    var html = '';
...
function buildMatrixToRemove(arr){
    ...
    return rId.filter(function(){ return true; });
}

function rules(matrix, mKey, id) {
    var rId = null;
    ...
    // clear blocks
    if(rId !== null) matrix = clearBlocks(rId, matrix);
}
...
$(document).on('click', '.tic-btn', function() {
    ...
    var leftIds = removeFromArray(allIds, id);

	// remove unneeded blocks
    rules(leftIds, mKey, id);
